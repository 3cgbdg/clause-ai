...
import {
  Type, AlertCircle, CheckCircle, Loader2,
  FileText, Share2, RefreshCcw, Copy,
} from 'lucide-react';
...
const Analyzer = () => {
  ...
  const handleAnalyze = () => {
    if (inputMode === 'text') {
      ...
      toast(`Analyzing "${file.name}"…`, 'info');
      analyze({ file });
    }
  };
  ...
  const showResults = status === 'analyzing' || status === 'completed' || status === 'error';
  ...
  return (
    <>
      <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-fade-in">
        ...
        {/* INPUT CARD */}
        <div className="glass p-8 rounded-3xl border-white/10 shadow-2xl mb-6">
          {/* Mode Toggle */}
          ...
          {inputMode === 'text' ? (
            <div className="relative group">
              <textarea
                ...
                className="w-full h-52 bg-black/20 border border-white/10 rounded-2xl p-6 text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-accent/40 resize-none font-mono text-sm custom-scrollbar transition-all group-hover:border-white/20"
              />
          ...
          <button
            ...
            className={`w-full mt-8 py-5 rounded-2xl flex items-center justify-center gap-3 font-bold text-lg transition-all active:scale-95 ${
              isAnalyzeDisabled
        ...
        {/* RESULTS — appear below input as a single column block */}
        {showResults && (
          <div className="animate-fade-in">
            {error ? (
              <ErrorBanner message={error} onRetry={handleAnalyze} onReset={handleReset} />
            ) : (
              <div className="glass p-8 rounded-3xl flex flex-col gap-8 relative overflow-hidden border-white/10 shadow-2xl">
                {/* Shimmer progress bar while analyzing */}
                {status === 'analyzing' && (
                  <div className="absolute top-0 left-0 w-full h-[3px] bg-accent/10 overflow-hidden">
                    <div
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      {status === 'analyzing' && (
                        <span className="text-[10px] text-accent animate-pulse font-bold uppercase tracking-wider">Streaming Live</span>
                      )}
                ...
                {status === 'completed' && (
                  <div className="flex gap-4 pt-6 border-t border-white/5">
                    <button
                      ...
                      className="flex-1 py-3 px-4 rounded-xl glass border border-white/5 text-sm font-bold flex items-center justify-center gap-2 hover:bg-white/5 transition-all active:scale-95 text-gray-300"
                    >
        ...
        <style>{`
          ...
          .dot-1 { animation: dotpulse 1.5s infinite 0s; }
